...
answer(trim, X) :- trim("  Hello Deriva  ", X).
answer(lower, X) :- lowercase("Hello Deriva", X).
answer(upper, X) :- uppercase("Hello Deriva", X).
...
answer(atom_string, X) :- atom_string(deriva, X).
